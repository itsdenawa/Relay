...
Each preview must pass the public landing and login smoke, mobile reflow, command-palette navigation, and accessibility checks. Stateful collaboration E2E continues to run against the disposable local Supabase stack in CI.
...
Run `pnpm release:check`, deploy the database migration, deploy Vercel, and then run `pnpm test:smoke:production` with `PRODUCTION_URL`. GitHub also starts the smoke suite for successful Vercel Production deployment events. The smoke suite verifies `/`, `/api/health`, production security headers, public accessibility, 320 px reflow, and smoke-account sign-in. Manual release validation also covers `/login`, `/signup`, `/onboarding`, `/w/[slug]`, `/w/[slug]/inbox`, `/w/[slug]/projects`, a project board in board and list views, and workspace/profile settings.
